...
![Imagem](/TrabalhoFinal/Imagens/DS_Process.png)
...
![Esboço de gráfico](/TrabalhoFinal/Imagens/GráficoEsperado-Grey.png)
...
|-------------------|---------------------------|-----------|---------------------|
| Regiao - Sigla    | alfanumerico              | Código da região brasileira em que se está localizado a revenda | "CO", "N", "NE", "S", "SE" |
| Estado - Sigla    | alfanumerico              | Código do estado brasileiro em que se está localizado a revenda | "ES", "GO", "TO"|
| Municipio         | alfanumerico              | Nome do município da revenda | "JAPARATUBA", "CARAGUATATUBA", "INDAIATUBA", "UBATUBA" |
| [Revenda][ExpRev] | alfanumerico              | Nome da revenda | "ZURANO AUTO POSTO LTDA", "MIMIM COMERCIO DE COMBUSTIVEIS EIRELI", "MINIANO COMBUSTIVEIS LTDA" |
| CNPJ da Revenda   | [alfanumerico*][ExpAlfa]  | Número do Cadastro Nacional de Pessoa Jurídica | "00.000.042/0001-22",  "00.000.042/0002-03" |
| Nome da Rua       | alfanumerico              | Nome do logradouro | " IRINEU PINTO", "GLEBA", "LADEIRA DO CAMURUJIPE3" |
| Numero Rua        | alfanumerico              | Número do logradouro | "0,041666667", 90130, 229000 |
| Complemento       | alfanumerico              | Complemento do logradouro | "-50", "KM 364+40 METROS", "FUNDOS PARA JACU PESSEGO Nº 4700" |
| Bairro            | alfanumerico              | Nome do bairro  | ÁGUA: BRANCA, DE MENINOS, DOCE, FRESCA, FRIA, LIMPA, PARADA, PRETA, QUENTE, RASA, RAZA, SANTA, VERDE |
| Cep               | alfanumerico              | Número do Código do Endereço Postal (CEP) do logradouro | 00000-000, 01000-000, 99935-800 |
| Produto           | alfanumerico              | Nome do combustível  | "GASOLINA", "ETANOL", "DIESEL", "GNV", "DIESEL S50", "DIESEL S10", "GASOLINA ADITIVADA" |
...
| Unidade de Medida | alfanumerico              | Unidade de Medida | "R$ / litro", "R$ / m³" |
| Bandeira          | alfanumerico              | Nome da [Bandeira][ExpBnd] da revenda | "BRANCA", "COSAN LUBRIFICANTES", "MAGNUM", "WALENDOWSKY", "TORRAO", "TRIANGULO" |
...
[ExpAlfa]: "Nos metadados, o CNPJ é tido como numerico, mas ele me parece mais um alfanumerico por conter vírgula, ponto e hífen"
[ExpRev]: "'Posto de combustivel' ou 'local onde há o comércio de combustível'"
...
Antes de realizarmos quaisquer operações nos dados, primeiro precisamos instalar e importar a biblioteca que serão utilizada para a manipulação dos dados. O [tidyverse][LinkTDVS] é uma coleção de diversos pacotes em R que facilitam a manipulação dos dados. Já o lubridate é uma biblioteca (supostamente pertencente ao tidyverse, mas que apresentou problemas durante a execução) que trás funções apropriadas para a manipulação de datas.
...
Nessa etapa, iremos realizar a "aquisição" dos dados. Tomandos os dados de seu fonte de origem e o estruturando de uma forma específica no código para que possamos trabalhar com eles. Mas antes de atingirmos o molho, precisamos primeiro preparar os tomates.

![Molho de Tomate](/Imagens/tomate.gif)
...
Obs.: perceba que caso todos os blocos de código deste arquivos sejam executados sequencialmente, essa opção irá sobrepor a opção de importação pelo link.
...
2. Os nomes da coluna não estão descritas da forma ideal para a análise dos dados, dessa forma podemos utilizar o parâmetro `name_repair` com o valor "universal" para corrigirmos, neste caso efetivamente substituindo hífens e espaços por pontos (".").
...
ggplot(transformed_db, aes(x = data, y = valor, color = presidentes)) +
  ...
  scale_color_manual(
    values = c("#EDDA05", "#454545", "#76026A", "#8A007C", "#800000", "#FF0000")
  )
```

#### Salvar a imagem

Por fim, salvemos a imagem 🖼️! Eu utilizei um índice (atualmente em 44) pois fui gerando diversas imagens de teste ao longo do processo de testes.

```{r}
index <- 44
# getwd() and change it is necessary using setwd(path)
```
```{r}
index <- index + 1
ggsave(
  # filename = paste0("meu_grafico_", index, ".png"),
  filename = paste0("meu_grafico_50.png"),
  path = "",
  width = 8,
  height = 5,
  dpi = 300
...
Nesta etapa busco trazer parte da sequência de gráficos gerados em busca do gráfico final desejado, ou próximo o bastante do que se deseja.

#### Prova de conceito

Este primeiro gráfico representa uma das primeiras tentativas de expor parte dos dados tratados de forma coerente.

![Grafico](/TrabalhoFinal/Imagens/meu_grafico_1.png "Grafico")

Mesmo que não chegue próximo do final, já conseguimos ver a tendência "natural" ao crescimento do preço do combustível.

#### Etapas marcantes

Aqui trago alguns dos avanços que considerei mais relevantes durante o desenvolvimento.

##### Quantidade mais elevada de dados
![Grafico](/TrabalhoFinal/Imagens/meu_grafico_10-por_dia.png "Grafico")
##### Coloração por presidente
![Grafico](/TrabalhoFinal/Imagens/meu_grafico_15.png "Grafico")
##### Gráfico por tempo de governo
![Grafico](/TrabalhoFinal/Imagens/meu_grafico_28.png "Grafico")
...
A importação, embora possa ter um caminho simples e direto, tem um considerável número de "armadilhas". Algumas delas são:
...
Tendo sido realizada uma boa organização prévia, a manipulação para se alcançar a tabela final desejada não apresentou muitos problemas.
...
Entretanto, pode-se perceber o visível aumento dos preços dos combustíveis, independente de qual governante estava em atuação.
...
Temospor fim este belíssimo gráfico 🤩:

![/Imagens/meu_grafico_43.png]